...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("Grados máx. de ángulo inicial respecto al eje X")]
    float serveMaxAngleDeg = 20f;
    [SerializeField, Tooltip("Evita direcciones casi horizontales (X muy pequeño)")]
    float minAbsXDir = 0.6f;

    [Header("Spin / Control")]
    [SerializeField, Tooltip("Cuánto afecta la posición del impacto en Y")]
    ...
    {
        // Dirección en X hacia izquierda o derecha
        ...
        // Asegurar componente X mínimo (evita rallies eternos verticales)
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        {
            // Refleja usando la normal (funciona para paredes inclinadas también)
        ...
        if (col.collider.CompareTag("Paddle"))
        {
            ...
            // Dirección: invierte X, ajusta Y por punto de impacto (+ pequeño jitter)
        ...
        // Cualquier otro collider: reflexión genérica
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            onGoal?.Invoke("Left");   // Punto contra la izquierda (anotó la derecha)
            ResetBall();
            Invoke(nameof(ServeToRight), serveDelay);
        }
        else if (other.CompareTag("GoalRight"))
        {
            onGoal?.Invoke("Right");  // Punto contra la derecha (anotó la izquierda)
            ResetBall();
